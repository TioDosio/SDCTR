...
float H = 0, y = 0, u = 0, r{10}, run_time = 0, run_init = 0, energy = 0, power_max = ((0.29 * 0.29) / 47) + ((0.29/47) * 2.63), vcc = 3.3;
int occupied = 1, anti_windup = 1, feedback = 1, flag_stream_lux = 0, flag_stream_duty_c = 0, Led = 1,counter = 1, bumpless=1, flag_dc=0;
double sum_visibility_error = 0, sum_flicker = 0, mean_flicker = 0, mean_visibility = 0,mean_energy=0, dc_1 = 0, dc_2 = 0;
...
void loop()
{
  ...
  if (currentTime - previousTime >= sampInterval)
  {
    ...
    if (occupied == 1 and r < 10) // meter bem
    {
      r = 15;
    }
    else if (occupied == 0 and r >= 5)
    {
      r = 5;
    }
  ...
  analogWrite(LED_PIN, pwm); // write pwm value to led
    if (flag_stream_lux != 0) // strem lux values of the desk
    {
      ...
      Serial.print(y);
      Serial.print(" ");
      Serial.println(currentTime/1000000);
    }
    ...
    {
      addToBuffer(ext, duty_cycle, r, y);
